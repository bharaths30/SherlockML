...
def main():
	(trainingFeatures,labels)=getTrainingDataFromCSV()
	noOfRows=len(trainingFeatures)
	baseInsert='INSERT INTO training VALUES (0.0,0.0,\"unknown\",'
	i=0
	...
	while i<noOfRows:
		oneDataSetStr=""
		for each in trainingFeatures[i]:
			oneDataSetStr=oneDataSetStr+str(each)+","
		classVal="KaggleUser_"+str(labels[i])
		row=oneDataSetStr+"\""+classVal+"\""
		insertQuery=baseInsert+row+");"
		print i
